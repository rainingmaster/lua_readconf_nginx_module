...
static int ngx_http_lua_readconf_get_lua_main_conf(lua_State * L);
...
ngx_http_lua_readconf_create_module(lua_State * L)
{
    ...
    lua_pushcfunction(L, ngx_http_lua_readconf_get_lua_main_conf);
    lua_setfield(L, -2, "get_lua_main_conf");
...
static int
ngx_http_lua_readconf_get_lua_main_conf(lua_State * L)
{
    ...
    lua_pushliteral(L, lmcf->lua_path);
